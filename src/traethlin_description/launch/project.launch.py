...
def generate_launch_description():
  ...
  pkg_alw82_major_project = get_package_share_directory('alw82_major_project')

  traethlin_urdf = Command(['xacro', ' ', os.path.join(pkg_traethlin_description,
                                                      'urdf',
                                                      'traethlin.urdf.xacro')])

  leader_urdf = Command(['xacro', ' ', os.path.join(pkg_alw82_major_project,
                                                      'urdf',
                                                      'leader.urdf.xacro')])
  ...
  return LaunchDescription([
    ...
    Node(
        ...
        name='urdf_spawner',
        output='screen',
        arguments=["-robot_namespace", namespace_2,
                   "-topic", ["/", namespace_2, "/robot_description"],
                   "-entity", "leader", "-x", "2", "-y", "0", "-z", "0"]
    )
  ])
